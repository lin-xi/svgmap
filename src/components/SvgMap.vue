...
export default {
  ...
  methods: {
    loadSvg() {
      fetch(this.src).then(res => res.text()).then(result => {
        console.log(result)
      })
    },
    init() {
      ...
      container.width = width * window.devicePixelRatio
      container.height = height * window.devicePixelRatio
    ...
    addGeoObject(group, svgObject) {
      ...
      for (let i = 0; i < paths.length; i++) {
        const path = this.transformSVGPath(paths[i]);
